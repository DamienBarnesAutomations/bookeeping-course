# Learning Outcome 1 — Payroll Terminology

**Objective:** Explain the key terminology associated with personal taxation in preparing and maintaining payroll records both manually and on the computer.

---

## Core Payroll Concepts

### Gross Pay
Total earnings before any deductions.

- **Basic Pay** — regular wages/salary
- **Overtime** — hours worked beyond normal (time + 1/3, time + 1/2, double time)
- **Shift Allowance** — extra pay for shift work
- **Bonus / Commission** — performance-related pay
- **Holiday Pay** — paid annual leave
- **Sick Pay** — company sick pay scheme
- **PRSI (Pay Related Social Insurance)** — social insurance contribution

### Statutory Deductions
- **PAYE (Pay As You Earn)** — income tax deducted at source
- **PRSI** — social insurance (Employee + Employer)
- **USC (Universal Social Charge)** — additional tax on gross income

### Non-Statutory Deductions
- Pension contributions
- Union dues
- Health insurance
- AVCs (Additional Voluntary Contributions)

### Net Pay (Take-Home Pay)
Gross Pay minus all deductions.

```
Net Pay = Gross Pay − (PAYE + PRSI + USC + Other Deductions)
```

---

## Tax Terminology

| Term | Definition |
|------|------------|
| **Tax Year** | 1 January to 31 December |
| **Tax Credits** | Amount deducted from tax owed (e.g., Single Person Tax Credit: €1,875) |
| **Standard Rate Cut-Off Point** | Income taxed at 20% up to this amount (e.g., €42,000 for single person) |
| **Higher Rate** | Income above cut-off taxed at 40% |
| **Tax Bands** | 20% (standard) and 40% (higher) |
| **Cumulative System** | Tax calculated on cumulative earnings to date each pay period |
| **Week 1 / Month 1 System** | Tax calculated on each period in isolation (emergency basis) |
| **Emergency Tax** | Temporary tax when employer lacks employee's tax details |

### 2025/2026 Key Figures (Indicative)

| Item | Single | Married (1 income) | Married (2 incomes) |
|------|--------|-------------------|-------------------|
| Standard Rate Cut-Off | ~€42,000 | ~€51,000 | ~€51,000 + transferable |
| Personal Tax Credit | ~€1,875 | ~€3,750 | ~€1,875 each |
| Employee PRSI (4%) | €352+ p.w. | €352+ p.w. | €352+ p.w. |
| USC Rates | 0.5% to 8% | 0.5% to 8% | 0.5% to 8% |

---

## Payroll Records to Maintain

| Record | Purpose |
|--------|---------|
| Employee Register | Names, PPS numbers, addresses, start dates |
| Time Records | Hours worked, overtime, absences |
| Payslips | Breakdown of pay and deductions per employee |
| Payroll Summary | Total gross, deductions, net per pay period |
| P30 (Monthly) | Monthly PAYE/PRSI/USC return to Revenue |
| P35 (Annual) | Annual employer return — **abolished 1 January 2019** under PAYE Modernisation; replaced by real-time payroll submissions via ROS |
| P60 (Annual) | End-of-year certificate for each employee |
| P45 | Leaver certificate — **abolished 1 January 2019** under PAYE Modernisation; employers now notify Revenue via a payroll submission with the leaving date; employees access records via myAccount |

> **PAYE Modernisation (2019):** From 1 January 2019, employers submit payroll data to Revenue in real time on or before each pay date. The P35 annual return and the old paper P45 leaver workflow were abolished. Revenue Payroll Notifications (RPNs) replaced the old Tax Credit Certificate (TCC).


---

## Worked Examples & Deep Dive

### How PAYE Actually Works

Revenue gives each employee:
- A **tax credit** (e.g., €3,750/year) — a deduction from tax owed, NOT from income
- A **standard rate cut-off point** (SRCOP) (e.g., €42,000/year) — income taxed at 20% up to this amount

**Example:** Employee earning €45,000 with €3,750 credits and €42,000 SRCOP.

```
Tax calculation:
  First €42,000 @ 20%        = €8,400.00
  Remaining €3,000 @ 40%     = €1,200.00
  Total gross tax             = €9,600.00
  Less tax credits           (€3,750.00)
  PAYE due                    = €5,850.00
```

The €3,750 tax credit reduces the tax from €9,600 to €5,850. It is NOT a deduction from the €45,000 salary.

### Common Misconception

"I earn €45,000 and have a €42,000 cut-off and €3,750 credit"

Wrong: "I pay tax on €45,000 minus €42,000 = €3,000 at 40%, and €42,000 at 20%, less €3,750."
Right: "I pay 20% on first €42,000 = €8,400, 40% on remaining €3,000 = €1,200. Total tax €9,600 less €3,750 credits = €5,850."

### Revenue Payroll Notification (RPN) Explained

The RPN is the modern replacement for the old Tax Credit Certificate (TCC).

Flow:
```
Revenue -> (electronically) -> Employer's Payroll Software -> Applied to employee
```

What an RPN contains:
- Employee's tax credits (e.g., €72.12/week)
- Employee's SRCOP (e.g., €807.69/week)
- Tax basis (Cumulative or Week 1)
- PRSI class
- USC rate bands
- Any Revenue-ordered adjustments

Without an RPN, the employer MUST use emergency tax. The employee ends up overtaxed and waits for a refund.

### Practice

Calculate PAYE for: Gross €38,000, SRCOP €42,000, Credits €3,750.

Since €38,000 < €42,000, all taxed at 20%:
Tax = €38,000 x 20% = €7,600
Less credits = €7,600 - €3,750 = **€3,850 PAYE**
